...
library(neuralnet)
red_neural=neuralnet(V18~V1+V2+V3+V4+V5+V6+V7+V8+V9+V10+V17+NL1+NL2+NL3+NL4+NV1+NV2+NV3,data = training_set ,hidden = 3,act.fct = 'logistic',linear.output = FALSE)
plot(red_neural)
red_neural$weights
## prueba matriz red neuronal
...
results<-data.frame(actual=training_set$V18, prediction=red_neural$net.result)
results

predicted=results$prediction * abs(diff(range(Deslizamientos))) + min(Deslizamientos)
actual=results$actual * abs(diff(range(Deslizamientos))) + min(Deslizamientos)
...
pred1<-predict(Modelo2,test_set, type-'class')
